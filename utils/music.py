...
class Player(commands.Cog):
    ...
    async def play(self, ctx, *, song=None):
        ...
        if ctx.voice_client.source is not None:
            ...
            if queue_len < 100:
                ...
                return await ctx.send("Sorry, I can only queue up to 100 songs, please wait for the current song to finish.")
    ...
    async def search(self, ctx, *, song=None):
        ...
        info = await self.search_song(20, song)
    ...
    async def skip(self, ctx):
        ...
        poll = discord.Embed(title=f"Vote to Skip Song by - {ctx.author.name}#{ctx.author.discriminator}", description="**60% of the voice channel must vote to skip for it to pass.**", colour=discord.Colour.blue())
        poll.add_field(name="Skip", value=":white_check_mark:")
        poll.add_field(name="Stay", value=":no_entry_sign:")
        poll.set_footer(text="Voting ends in 20 seconds.")
        ...
        if votes[u"\u2705"] > 0:
            if votes[u"\U0001F6AB"] == 0 or votes[u"\u2705"] / (votes[u"\u2705"] + votes[u"\U0001F6AB"]) > 0.59: # 60% or higher
                skip = True
